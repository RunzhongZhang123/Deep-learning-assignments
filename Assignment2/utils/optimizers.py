...
class RMSpropOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        ###################################################
        for k in grads:
            cache[k] = (1 - gamma) * (grads[k]**2) + gamma * cache[k]
            params[k] = params[k] - learning_rate * grads[k] / (eps + np.sqrt(cache[k]))
...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        ###################################################
        for k in grads:
            momentums[k] = (1 - beta1) * grads[k] + momentums[k] * beta1
            velocitys[k] = (1 - beta2) * (grads[k] ** 2) + velocitys[k] * beta2
            t += 1
            momentums_tmp = momentums[k] / (1 - (beta1 ** t))
            velocitys_tmp = velocitys[k] / (1 - (beta2 ** t))
            params[k] = params[k] - momentums_tmp * learning_rate / (eps + np.sqrt(velocitys_tmp))
