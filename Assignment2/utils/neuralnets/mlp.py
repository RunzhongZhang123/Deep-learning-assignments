...
class MLP(object):
    ...
    def loss(self, X, y):
        ...
        for i in range(num_layers - 1):
            ...
            if use_bn:
                ...
                x, cache[cache_name] = bn_forward(x, params["bn_gamma_{}".format(i)], params["bn_beta_{}".format(i)], bn_params[i], mode = 'train')
                bn_params[i]
            ###############################################
        ...
        for i in range(num_layers - 1):
            ...
            #raise NotImplementedError
            if dropout_config["enabled"]:
                dx = dropout_backward(dx, cache['batch_{}'.format(j)])
            ###############################################
            # END OF DROPOUT                              #
            ###############################################     
            # ReLU backward
    ...
    def predict(self, X):
        ...
        for i in range(num_layers - 1):
            ...
            #raise NotImplementedError
            if use_bn:
                x, _ = bn_forward(x, params["bn_gamma_{}".format(i)], params["bn_beta_{}".format(i)], bn_params[i], mode = 'test')
            ###############################################
            # END OF BATCH NORMALIZATION                  #
            ###############################################   
            # ReLU
