...
def my_training_task4(X_train, y_train, X_val, y_val, 
    ...
    gen = ImageGenerator(X_train.reshape(-1,1,28,28).transpose(0,2,3,1), y_train)
    translated = gen.translate(5,0)
    rotated = gen.rotate(30)
    flipped = gen.flip(mode = 'hv')
    X_train = np.concatenate((X_train,translated, rotated, flipped), axis = 0)
