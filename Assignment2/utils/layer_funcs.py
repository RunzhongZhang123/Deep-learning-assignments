...
def conv2d_forward(x, w, b, pad, stride):
    ...
    new_height = ((height - filter_height + 2 * pad) // stride) + 1
    new_width = ((width - filter_width + 2 * pad) // stride) + 1
    x = np.pad(x, ((0,), (pad,), (pad,), (0,)), constant_values=0, mode='constant')
    y = np.zeros((batch, new_height, new_width, num_of_filters))

    for B in range(batch):
        for f in range(num_of_filters):
            for h in range(new_height):
                for W in range(new_width):
                    y[B, h, W, f] = np.sum(x[B, h*stride:h*stride+new_height, W*stride:W*stride+new_width, :] * w[:, :, :, f]) + b[f]
...
def conv2d_backward(d_top, x, w, b, pad, stride):
    ...
    for B in range(batch):
        for F in range(num_of_filters):
            d_b[F] = d_b[F] + d_top[B, :, :, F].sum()
            for H in range(new_height):
                for W in range(new_width):
                    d_w[:, :, :, F] = d_w[:, :, :, F] + x[B, H*stride:new_height+H*stride, W*stride:new_width+W*stride, :] * d_top[B, H, W, F]
...
def max_pool_forward(x, pool_size, stride):
    ...
    y = np.zeros((batch, new_height, new_width, channels))

    for B in range(batch):
        for C in range(channels):
            for H in range(new_height):
                for W in range(new_width):
                    y[B, H, W, C] = np.amax(x[B, H*stride:new_height+H*stride, W*stride:new_width+W*stride, C])
    return y
    #######################################################################
...
def max_pool_backward(dout, x, pool_size, stride):
    ...
    for H in range(new_height):
        for W in range(new_width):
            x_new = x[:, H*stride:new_height+H*stride, W*stride:new_width+W*stride, :]
            tmp = (x_new == (np.max(x_new, axis=(1,2)))[:,None,None, :])
            dx[:, H*stride:new_height+H*stride, W*stride:new_width+W*stride, :] += (dout[:,H, W, :])[:,None,None, :] * tmp
    return dx
    #######################################################################
