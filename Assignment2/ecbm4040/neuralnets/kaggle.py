...
def my_LeNet(input_x, input_y, is_training,
            img_len=28,channel_num=1,output_size=5,
            conv_featmap=[6,16],fc_units=[84,50],conv_stride=[1,1],
...
def my_testing(X_test, pre_trained_model):
    img_len = X_test.shape[1]
    channel_num = X_test.shape[-1]
    res_valid = []
    # define the variables and parameter needed during training
    with tf.name_scope('inputs'):
        xs = tf.placeholder(shape=[None, img_len, img_len, channel_num], dtype=tf.float32)
    ...
    with tf.Session() as sess:
        ...
        # try to restore the pre_trained model
        try:
            print("Load the model from: {}".format(pre_trained_model))
            saver.restore(sess, 'KaggleModel/{}'.format(pre_trained_model))
        except Exception:
            raise ValueError("Load model Failed!")

        ans = sess.run([output], feed_dict={xs: X_test})
        ans = np.reshape(ans, (X_test.shape[0], 5))
