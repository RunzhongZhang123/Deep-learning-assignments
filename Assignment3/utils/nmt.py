...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob, 
    ...
    ######################################################################################
    if cell_type == 'LSTM':
        stack = tf.contrib.rnn.MultiRNNCell([tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.LSTMCell(rnn_size), keep_prob) for _ in range(num_layers)]) 
    elif cell_type == 'GRU':
        stack = tf.contrib.rnn.MultiRNNCell([tf.contrib.rnn.DropoutWrapper(tf.contrib.rnn.GRUCell(rnn_size), keep_prob) for _ in range(num_layers)])
    rnn_outputs, rnn_final_state = tf.nn.dynamic_rnn(stack, embed, dtype=tf.float32)
...
def my_optimizer(loss,grad_clip, learning_rate):
    ...
    gradient_clip = [(tf.clip_by_value(grad, -1*grad_clip, grad_clip), var) for grad, var in gradients if grad is not None]
    ans = optimizer.apply_gradients(gradient_clip)
    return ans
